...
For each compound, pulls papers mentioning it + Parkinson's, builds a
graph where papers are linked if they share an author, and computes an
"effective independent evidence count" (number of connected components)
instead of trusting the raw paper count.
"""
import os, time, requests, pandas as pd, networkx as nx
...
def fetch_authors(pmids):
    """Batch fetch author lists for a list of PMIDs in one call."""
    if not pmids:
        return {}
    ...
    for article in root.findall(".//PubmedArticle"):
        ...
        result[pmid] = {"authors": authors, "year": year, "title": title}
    return result

def effective_independent_count(paper_authors):
    """Build graph: papers as nodes, edge if they share >=1 author.
    Effective count = number of connected components."""
    G = nx.Graph()
...
def main():
    ...
    with open(CHECKPOINT_FILE, "a") as ckpt:
        for compound in tqdm(compounds):
            ...
            try:
                ...
                if raw_count == 0:
                    eff_count, n_components = 0, 0
                else:
                    n_components, _ = effective_independent_count(paper_authors)
    ...
    # Reload everything from checkpoint (covers resumed + fresh rows)
    all_rows = []
    with open(CHECKPOINT_FILE) as f:
        for line in f:
    ...
    print("\nTop 10 by raw paper count (see how independence corrects them):")
